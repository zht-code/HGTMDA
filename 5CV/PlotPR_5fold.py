...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
storFile(FprAndTpr, '5CV/5foldPR_FprAndTprRPC.csv')#这个文件是为了和别的。消融实验的模型的结果比较，用来绘制比较的PR图


plt.plot(mean_tpr, mean_fpr, color=colorlist[5],
...
# # 保存图片
plt.savefig('PR-5fold.pdf',dpi=310)
plt.show()