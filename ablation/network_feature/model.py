...
class MDA(nn.Module):
    ...
    def forward(self, data, train_sample, test_sample, device):
        ...
        for i in range(len(train_sample)):
            a = torch.cat((data['miRNA_disease_embedding'][train_sample[i][0]], data['miRNA_disease_embedding'][train_sample[i][1]]), dim=0).unsqueeze(0)
        ...
        for i in range(len(test_sample)):
            a = torch.cat((data['miRNA_disease_embedding'][test_sample[i][0]], data['miRNA_disease_embedding'][test_sample[i][1]]), dim=0).unsqueeze(0)
